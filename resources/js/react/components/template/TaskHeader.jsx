...
const TaskHeader = () => {
    ...
    useEffect(() => {
        const clickHandler = ({ target }) => {
            ...
            if (!popupOpen || popup.current.contains(target) || trigger.current.contains(target))
                return
            setPopupOpen(false)
        }
    ...
    return (
        <div className="flex flex-col gap-y-4 rounded-sm border border-stroke bg-white p-3 shadow-default sm:flex-row sm:items-center sm:justify-between dark:border-strokedark dark:bg-boxdark">
            <div>
